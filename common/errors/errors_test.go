	...
	for i := 0; i < len(tc); i++ {
		t.Run(tc[i].name, func(t *testing.T) {
			...
			if tc[i].generateStack[0] {
				...
				assert.Contains(err.Error(), "github.com/slava-balahontsev/bcnex/common/errors", "Test case '%s' failed", tc[i].name)
			} else {
				if tc[i].messageArgs != nil {
				...
				assert.NotContains(err.Error(), "github.com/slava-balahontsev/bcnex/common/errors", "Test case '%s' failed", tc[i].name)
			}
			...
			assert.NotContains(err.Message(), "github.com/slava-balahontsev/bcnex/common/errors", "Test case '%s' failed", tc[i].name)
			...
			if tc[i].wrappedErrorArgs != nil {
				if len(tc[i].wrappedErrorArgs) == 3 {
					if tc[i].generateStack[1] {
						...
						assert.Contains(wrappedErr.Error(), "github.com/slava-balahontsev/bcnex/common/errors", "Test case '%s' failed", tc[i].name)
					} else {
						if tc[i].wrappedMessageArgs != nil {
						...
						assert.NotContains(wrappedErr.Error(), "github.com/slava-balahontsev/bcnex/common/errors", "Test case '%s' failed", tc[i].name)
					}
