...
The atomic broadcast ordering protocol for Hyperledger Fabric is described in `slava-balahontsev/bcnex/protos/orderer/ab.proto`. There are two services: the `Broadcast` service for injecting messages into the system and the `Deliver` service for receiving ordered batches from the service.
